...
public class Game {
    ...
    public void loadGame(GameID gameID) {
        if (gameLevel == null || gameLevel.getLives() <= 0) {
            gameLevel = new GameLevel(gameLevels.get(gameID.difficulty).get(gameID.level));
        } else {
            // Preserve current lives when changing levels
            int currentLives = gameLevel.getLives();
            gameLevel = new GameLevel(gameLevels.get(gameID.difficulty).get(gameID.level));
            // Restore lives from previous level
            while (gameLevel.getLives() > currentLives) {
    ...
    private void readLevels(){
        InputStream is;
        is = ResourceLoader.loadResource("levels.txt");
